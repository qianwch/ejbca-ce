
-- ServiceData gets two new columns
ALTER TABLE ServiceData ADD nextRunTimeStamp BIGINT DEFAULT 0 NOT NULL;
ALTER TABLE ServiceData ADD runTimeStamp BIGINT DEFAULT 0 NOT NULL;

-- Add rowVersion column to all tables
ALTER TABLE AccessRulesData ADD COLUMN rowVersion INTEGER DEFAULT 0; 
ALTER TABLE AdminEntityData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE AdminGroupData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE AdminPreferencesData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE ApprovalData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE AuthorizationTreeUpdateData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE CAData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE CRLData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE CertReqHistoryData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE CertificateData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE CertificateProfileData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE EndEntityProfileData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE GlobalConfigurationData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE HardTokenCertificateMap ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE HardTokenData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE HardTokenIssuerData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE HardTokenProfileData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE HardTokenPropertyData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE KeyRecoveryData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE LogConfigurationData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE LogEntryData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE PublisherData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE ServiceData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE UserData ADD COLUMN rowVersion INTEGER DEFAULT 0;
ALTER TABLE UserDataSourceData ADD COLUMN rowVersion INTEGER DEFAULT 0;

-- Add rowProtection column to all tables
ALTER TABLE AccessRulesData ADD COLUMN rowProtection VARCHAR DEFAULT NULL; 
ALTER TABLE AdminEntityData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE AdminGroupData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE AdminPreferencesData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE ApprovalData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE AuthorizationTreeUpdateData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE CAData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE CRLData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE CertReqHistoryData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE CertificateData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE CertificateProfileData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE EndEntityProfileData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE GlobalConfigurationData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE HardTokenCertificateMap ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE HardTokenData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE HardTokenIssuerData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE HardTokenProfileData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE HardTokenPropertyData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE KeyRecoveryData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE LogConfigurationData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE LogEntryData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE PublisherData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE ServiceData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE UserData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
ALTER TABLE UserDataSourceData ADD COLUMN rowProtection VARCHAR DEFAULT NULL;
