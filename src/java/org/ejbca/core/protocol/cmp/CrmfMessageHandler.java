...
public class CrmfMessageHandler extends BaseCmpMessageHandler implements ICmpMessageHandler {
	...
	public CrmfMessageHandler(final Admin admin) throws CreateException, RemoteException {
		...
		this.signsession = ((ISignSessionHome)ServiceLocator.getInstance().getRemoteHome(ISignSessionHome.JNDI_NAME, ISignSessionHome.class)).create();
		this.usersession = ((IUserAdminSessionHome) ServiceLocator.getInstance().getRemoteHome(IUserAdminSessionHome.JNDI_NAME, IUserAdminSessionHome.class)).create();
		this.reqsession = ((ICertificateRequestSessionHome) ServiceLocator.getInstance().getRemoteHome(ICertificateRequestSessionHome.JNDI_NAME, ICertificateRequestSessionHome.class)).create();
	 ...
	 */
	private IResponseMessage handleRaMessage(final BaseCmpMessage msg, final CrmfRequestMessage crmfreq) throws RemoteException, AuthorizationDeniedException, EjbcaException, ClassNotFoundException {
		...
		try {
			...
			try {
				try {
					...
					resp = reqsession.processCertReq(admin, userdata, crmfreq, org.ejbca.core.protocol.cmp.CmpResponseMessage.class);
