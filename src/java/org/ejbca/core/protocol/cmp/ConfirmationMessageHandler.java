...
//	/** Parameter used to authenticate RA messages if we are using RA mode to create users */
	...
	public ResponseMessage handleMessage(BaseCmpMessage msg, boolean authenticated) {
		...
		if (version > 1) {
			...
			if (StringUtils.equals(responseProtection, "pbe") && (msg.getMessage().getProtectedBytes() != null) ) {
			    setPbeParameters(msg, authenticated, cresp);
			} else if (StringUtils.equals(responseProtection, "signature")) {
			    signResponse(msg, cresp);
			}
			resp = cresp;
			try {
				resp.create();
	...
	private void setPbeParameters(BaseCmpMessage msg, boolean authenticated, CmpConfirmResponseMessage cresp) {
	    
        String keyId = getSenderKeyId(msg.getHeader());
        CAInfo caInfo = null;
        try {
            int caId = CertTools.stringToBCDNString(msg.getRecipient().getName().toString()).hashCode();
	...
	private void signResponse(BaseCmpMessage msg, CmpConfirmResponseMessage cresp) {
        try {
