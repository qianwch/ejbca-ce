...
public class TestEndEntityProfile extends TestCase {
    ...
    public void test10CardnumberRequired() throws RemoteException, CertificateProfileExistsException, AuthorizationDeniedException {
        ...
        TestTools.getRaAdminSession().removeEndEntityProfile(admin, "TESTCARDNUMBER");
        EndEntityProfile profile = new EndEntityProfile();
        profile.addField(EndEntityProfile.CARDNUMBER);
        profile.setRequired(EndEntityProfile.CARDNUMBER, 0, true);
        profile.setValue(EndEntityProfile.AVAILCAS, 0, Integer.toString(TestTools.getTestCAId()));

        String cardnumber = "foo123";
        boolean ret = false;
        try {
            TestTools.getRaAdminSession().addEndEntityProfile(admin, "TESTCARDNUMBER", profile);
        } catch (EndEntityProfileExistsException pee) {}    
            
        profile = TestTools.getRaAdminSession().getEndEntityProfile(admin, "TESTCARDNUMBER");
            
        UserDataVO userdata = new UserDataVO("foo", "CN=foo", TestTools.getTestCAId(), "", "", SecConst.USER_ENDUSER, TestTools.getRaAdminSession().getEndEntityProfileId(admin, "TESTCARDNUMBER"), SecConst.CERTPROFILE_FIXED_ENDUSER, SecConst.TOKEN_SOFT_PEM, 0, null);
