...
public class CmpRAAuthenticationTest extends CmpTestCase {
	...
	private X509Certificate setupCA(String caName, String pbeSecret) throws Exception {
		LOG.trace(">setupCA");
		assertTrue("Failed to create " + caName, TestTools.createTestCA(caName, 512));
		X509CA x509Ca = (X509CA) TestTools.getCAAdminSession().getCA(ADMIN, TestTools.getTestCAId(caName));
		...
		TestTools.getCAAdminSession().editCA(ADMIN, x509CaInfo);
	...
	public void test04IssueConfirmRevokeEEP() throws Exception {
        ...
        // Create EEP
        if (TestTools.getRaAdminSession().getEndEntityProfile(ADMIN, EEP_1) == null) {
            ...
            eep.setValue(EndEntityProfile.DEFAULTCA, 0, "" + TestTools.getTestCAId(CA_NAME_1));
            eep.setValue(EndEntityProfile.AVAILCAS, 0, "" + TestTools.getTestCAId(CA_NAME_1));
            eep.setModifyable(DnComponents.RFC822NAME, 0, true);
            eep.setUse(DnComponents.RFC822NAME, 0, false);	// Don't use field from "email" data
            try {
    			TestTools.getRaAdminSession().addEndEntityProfile(ADMIN, EEP_1, eep);
	...
	public void testZZZCleanUp() throws Exception {
		...
		cleanUpOk &= TestTools.removeTestCA(CA_NAME_1);
		cleanUpOk &= TestTools.removeTestCA(CA_NAME_2);
		cleanUpOk &= TestTools.getConfigurationSession().restoreConfiguration();
		try {
			TestTools.getRaAdminSession().removeEndEntityProfile(ADMIN, EEP_1);
		} catch (Exception e) {
			LOG.error("", e);
			cleanUpOk = false;
		}
