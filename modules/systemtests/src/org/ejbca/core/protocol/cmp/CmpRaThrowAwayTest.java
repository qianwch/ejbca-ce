...
public class CmpRaThrowAwayTest extends CmpTestCase {
	...
	public void test000Setup() throws Exception {
		...
		caCertificate = (X509Certificate) TestTools.getCAAdminSession().getCA(ADMIN, TestTools.getTestCAId(TESTCA_NAME)).getCertificateChain().iterator().next();
	...
	public void testZZZTearDown() throws Exception {
		...
		cleanUpOk &= TestTools.getConfigurationSession().restoreConfiguration();
		cleanUpOk &= TestTools.removeTestCA(TESTCA_NAME);
	...
	public void testIssueConfirmRevoke(boolean useCertReqHistory, boolean useUserStorage, boolean useCertificateStorage) throws Exception {
		...
		assertEquals("Certificate history data was or wasn't stored: ", useCertReqHistory, TestTools.getCertificateStoreSession().getCertReqHistory(ADMIN, CertTools.getSerialNumber(cert), CertTools.getIssuerDN(cert))!=null);
		assertEquals("User data was or wasn't stored: ", useUserStorage, TestTools.getUserAdminSession().existsUser(ADMIN, username));
		assertEquals("Certificate data was or wasn't stored: ", useCertificateStorage, TestTools.getCertificateStoreSession().findCertificateByFingerprint(ADMIN, CertTools.getFingerprintAsString(cert))!=null);
		...
		if (useCertificateStorage && useUserStorage) {
			...
			int reason = TestTools.getCertificateStoreSession().getStatus(CertTools.getSubjectDN(caCertificate), cert.getSerialNumber()).revocationReason;
		...
		if (useCertReqHistory) {
			TestTools.getCertificateStoreSession().removeCertReqHistoryData(ADMIN, CertTools.getFingerprintAsString(cert));
		}
		LOG.trace("<testIssueConfirmRevoke");
	}
