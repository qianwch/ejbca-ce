 ...
 * @version $Id$
 */
public class TestCustomCertSerialnumber extends TestCase {

	private static final Logger log = Logger.getLogger(TestCustomCertSerialnumber.class);
	...
	public TestCustomCertSerialnumber(String name) throws Exception {
		super(name);
	...
	protected void setUp() throws Exception {
		...
		fooCertProfileId = TestTools.getCertificateStoreSession().getCertificateProfileId(admin,"FOOCERTPROFILE");
		...
		fooEEProfileId = TestTools.getRaAdminSession().getEndEntityProfileId(admin, "FOOEEPROFILE");
	}    
	...
	public void test04CreateCertWithCustomSNNotAllowed() throws EndEntityProfileExistsException, InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, SignatureException, IOException, DuplicateKeyException, AuthorizationDeniedException, UserDoesntFullfillEndEntityProfile, EjbcaException, ClassNotFoundException, CertificateEncodingException, CertificateException, WaitingForApprovalException, InvalidAlgorithmParameterException {
		...
		CertificateProfile fooCertProfile = TestTools.getCertificateStoreSession().getCertificateProfile(admin, "FOOCERTPROFILE");
		fooCertProfile.setAllowCertSerialNumberOverride(false);
		TestTools.getCertificateStoreSession().changeCertificateProfile(admin, "FOOCERTPROFILE", fooCertProfile);
		...
		user.setExtendedinformation(ei);
		try {
			TestTools.getCertificateRequestSession().processCertReq(admin, user, p10, Class.forName(org.ejbca.core.protocol.X509ResponseMessage.class.getName()));
