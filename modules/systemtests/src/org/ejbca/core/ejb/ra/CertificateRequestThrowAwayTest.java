...
public class CertificateRequestThrowAwayTest extends TestCase {
	...
	public void test000Setup() throws RemoteException {
		...
		TestTools.createTestCA(TESTCA_NAME);	// Create test CA
	...
	public void testZZZTearDown() {
		...
		assertTrue("Clean up failed!", TestTools.removeTestCA(TESTCA_NAME));
		LOG.trace("<testZZZTearDown");
	}
	...
	private void generateCertificatePkcs10(boolean useCertReqHistory, boolean useUserStorage, boolean useCertificateStorage, boolean raw) throws AuthorizationDeniedException, UserDoesntFullfillEndEntityProfile, EjbcaException, InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, SignatureException, InvalidAlgorithmParameterException, CertificateEncodingException, CertificateException, IOException, RemoveException, InvalidKeySpecException, ObjectNotFoundException, CreateException {
		...
		if (useUserStorage) {
			TestTools.getUserAdminSession().deleteUser(admin, userData.getUsername());
		}
		if (useCertReqHistory) {
			TestTools.getCertificateStoreSession().removeCertReqHistoryData(admin, CertTools.getFingerprintAsString(certificate));
		}
		LOG.trace("<generateCertificatePkcs10");
	}
	...
	private void assertCAConfig(boolean useCertReqHistory, boolean useUserStorage, boolean useCertificateStorage) throws RemoteException {
		CAInfo caInfo = TestTools.getCAAdminSession().getCAInfo(admin, TESTCA_NAME);
	...
	private UserDataVO getNewUserData() {
		...
		UserDataVO userData = new UserDataVO(username, "CN="+username, TestTools.getTestCAId(TESTCA_NAME), null, null, UserDataConstants.STATUS_NEW, SecConst.USER_ENDUSER,
	...
	private Certificate doPkcs10Request(UserDataVO userData, boolean raw) throws AuthorizationDeniedException, UserDoesntFullfillEndEntityProfile, EjbcaException, NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException, InvalidKeyException, SignatureException, CertificateEncodingException, CertificateException, IOException, InvalidKeySpecException, ObjectNotFoundException, CreateException {
		...
		if (raw) {
			ret = CertTools.getCertfromByteArray(TestTools.getCertificateRequestSession().processCertReq(admin, userData, new String(Base64.encode(rawPkcs10req)), SecConst.CERT_REQ_TYPE_PKCS10, null, SecConst.CERT_RES_TYPE_CERTIFICATE));
		} else {
			...
			ret = ((X509ResponseMessage) TestTools.getCertificateRequestSession().processCertReq(admin, userData, pkcs10req, org.ejbca.core.protocol.X509ResponseMessage.class)).getCertificate();
		}
		return ret;
	...
	private boolean userDataExists(UserDataVO userData) throws RemoteException {
		return TestTools.getUserAdminSession().existsUser(admin, userData.getUsername());
	}

	private boolean certificateRequestHistoryExists(Certificate certificate) throws RemoteException {
		return TestTools.getCertificateStoreSession().getCertReqHistory(admin, CertTools.getSerialNumber(certificate), CertTools.getIssuerDN(certificate)) != null;
	}

	private boolean certificateExists(Certificate certificate) throws RemoteException {
		return TestTools.getCertificateStoreSession().getCertificateInfo(admin, CertTools.getFingerprintAsString(certificate)) != null;
	}
}
