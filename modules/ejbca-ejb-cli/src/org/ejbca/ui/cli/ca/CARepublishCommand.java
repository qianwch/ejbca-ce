...
public class CARepublishCommand extends BaseCaAdminCommand {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (data.getCertificateProfileId() > 0) { // only if we find a
                    ...
                    List<Certificate> certCol = ejb.getCertStoreSession().findCertificatesByUsername(data.getUsername());
    ...
    private void publishCert(AuthenticationToken authenticationToken, EndEntityInformation data, CertificateProfile certProfile, X509Certificate cert) {
        try {
            ...
            boolean ret = ejb.getPublisherSession().storeCertificate(authenticationToken, certProfile.getPublisherList(), cert, data.getUsername(), data.getPassword(), userDataDN,
