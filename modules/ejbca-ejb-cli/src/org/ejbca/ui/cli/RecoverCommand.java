...
public class RecoverCommand extends EjbcaCliUserCommandBase {
    ...
    public String getFullHelpText() {
        return "Recover certificates and end entities based on audit log events. This can be useful if you have lost " +
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(parameters.get(LOG_FILE_KEY)))) {
            for (String line; (line = bufferedReader.readLine()) != null;) {
                if (endEntityWasCreated(line)) {
                    recoverEndEntity(line, lineNumber);
                } else if (certificateWasCreated(line)) {
                    final boolean certificateWasRecovered = recoverCertificate(line, lineNumber);
     ...
     */
    private boolean recoverCertificate(final String line, final long lineNumber) throws AuthorizationDeniedException, CADoesntExistsException {
        try {
     ...
     */
    private boolean recoverEndEntity(final String line, final long lineNumber) throws AuthorizationDeniedException,
        ...
        try {
            ...
            if (subjectAltName.isPresent()) {
                endEntityInformation.setSubjectAltName(subjectAltName.get());
            }
